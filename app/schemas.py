...
# ── CHANGED: accepts courses list instead of track_scores ────────────────────
class CourseGrade(BaseModel):
    course_id: str = Field(..., example="CS-210")
    grade: float = Field(..., ge=0, le=100, example=90.0)
...
class TrackRecommendationRequest(BaseModel):
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                "courses": [
                    {"course_id": "CS-210", "grade": 90},
                    {"course_id": "CS-310", "grade": 85},
                ]
            }
        }
    )
    courses: List[CourseGrade] = Field(
        ...,
        min_length=1,
        description="List of course_id and grade pairs for the student.",
    )
# ─────────────────────────────────────────────────────────────────────────────
